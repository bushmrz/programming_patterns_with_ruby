...
class DBSourceAdapter
  ...
  def paginated_short_students(page, count, existing_data_list = nil)
    ...
    return DataListStudentShort.new(data: slice) if existing_data_list.nil?
  ...
  def add_student(student)
    template = 'INSERT INTO student(last_name, first_name, father_name, phone, telegram, mail, git) VALUES (?, ?, ?, ?, ?, ?, ?)'
    @db.prepare_exec(template, *student_fields(student))
    @db.query('SELECT LAST_INSERT_ID()').first.first[1]
  end

  def replace_student(student_id, student)
    template = 'UPDATE student SET last_name=?, first_name=?, father_name=?, phone=?, telegram=?, mail=?, git=? WHERE id=?'
  ...
  def student_fields(student)
    [student.last_name, student.first_name, student.father_name, student.phone, student.telegram, student.mail, student.git]
  end
end
